...
ensure_dir("./data")
ensure_file(
    "./data/%s" % WCS_CIELAB_FILE,
    "http://www1.icsi.berkeley.edu/wcs/data/cnum-maps/cnum-vhcm-lab-new.txt",
)
ensure_file(
    "./data/%s" % WCS_CNUM_FILE,
    "http://www1.icsi.berkeley.edu/wcs/data/20021219/txt/chip.txt",
)

__CHIPS = pd.read_csv("./data/%s" % WCS_CIELAB_FILE, delimiter="\t").sort_values(
    by="#cnum"
)
...
CNUMS_WCS_COR = dict(
    zip(
        WCS_CNUMS[:, 0],
        [
            (ROWS.index(WCS_CNUMS[cnum - 1, 1]), WCS_CNUMS[cnum - 1, 2])
            for cnum in WCS_CNUMS[:, 0]
        ],
    )
)
